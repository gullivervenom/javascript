...
/*
    배열의 reduce()
    - 배열을 순회하며 각 요소에 대하여 이전의 콜백 함수 실행 반환값을
...
/*
    배열의 some()
    - 배열 내 일부 요소가 콜백 함수의 테스트를 통과하는지 확인하여 결과를 boolean으로 반환
*/
...
let jjak = [1, 4, 5, 7, 9].some(item => item % 2==0);
console.log(`jjak : ${jjak}`);
...
result = student.find(a => a.name === '박재민');
console.log(result);

// student 배열에서 90점 이상인 학생 추출하기
result = student.filter(a => a.scroe >= 75);
console.log(result);
// find 를 쓰면 하나만 출력 되고 땡이다.
result = result.find(a => a.scroe >= 75);
console.log(result);